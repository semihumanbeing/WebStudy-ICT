...
<script type="text/javascript">
	function add_cart(){
		// 로그인 상태 체크하기
		if("${empty user}"=="true"){
			if(confirm('로그인 하신 후 장바구니 담기가 가능합니다.\n로그인 하시겠습니까?')==false) return;
			
			
			location.href="../member/loginForm.do?url="+encodeURIComponent(location.href);
			
			return;
		}
		// ajax 를 이용해서 장바구니에 값을 담는다
		$.ajax({
			url: 'cart_insert.do', // CartInsertAction
			data: {'p_idx':'${vo.p_idx}','m_idx':'${user.m_idx}'},
			success: function(res_data){
				// res_data = {'result' : 'success'} 등록성공시
				// res_data = {'result' : 'fail'} 등록실패시
				// res_data = {'result' : 'exist'} 이미 등록된 상태
