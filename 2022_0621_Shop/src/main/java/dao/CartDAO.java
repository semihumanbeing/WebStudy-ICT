...
public class CartDAO {
	...
	public List<CartVO> selectList(Map map) {
		List<CartVO> list = null;
	...
	public int selectTotalAmount(Map map) {
		int total_amount = 0;
	...
	public int deleteMap(Map map) {
		...
		SqlSession sqlSession = factory.openSession(true);
		res = sqlSession.delete("cart.deleteMap", map);
		sqlSession.close();

		return res;
	}

	public int selectMaxOrderIdx(int m_idx) {
		int max = 0;
		SqlSession sqlSession = factory.openSession();
		max = sqlSession.selectOne("cart.selectMaxOrderIdx", m_idx);
		sqlSession.close();
		return max;
	}
